...
- **Three LLM providers** — use [OpenRouter](https://openrouter.ai),
  [NanoGPT](https://nano-gpt.com), or Anthropic directly. OpenRouter and
  NanoGPT each cover research, hosted web search, fact-checking, writing, and
  TTS through one account; ElevenLabs is also supported for narration.
...
Go to **Settings → API keys**, paste an OpenRouter, NanoGPT, or Anthropic key,
and save it. The studio writes the key to the ignored local `.env` file and
binds only to `127.0.0.1`; do not expose the server to a LAN or public
interface. Choose the provider and its live model options under **Model
routing**, save, then create an episode. Start without audio if you want to
verify research and writing before paying for TTS.
...
# Then add OPENROUTER_API_KEY, NANOGPT_API_KEY, or ANTHROPIC_API_KEY to .env.
...
| --- | --- | --- |
| LLM provider | `OPENROUTER_API_KEY` / `NANOGPT_API_KEY` / `ANTHROPIC_API_KEY` (`LLM_PROVIDER` to force) | OpenRouter, then NanoGPT, then Anthropic |
...
| Voice provider | `VOICE_PROVIDER` | ElevenLabs if set, else active NanoGPT or OpenRouter gateway |
| Gateway TTS | `TTS_MODEL`, `TTS_VOICE`, `TTS_SPEED` | Provider-specific catalog default, `0.9×` |

Use the exact IDs shown in the selected provider's live dropdown. Direct
Anthropic uses bare model IDs; OpenRouter and NanoGPT use their catalog IDs.
...
when the provider reports per-call cost (OpenRouter supplies exact cost data).
